...
def write_ply_with_edges(
    ...
    """以 ASCII PLY 写出点云 + 线框（线框用三角面片表示，MeshLab 可渲染颜色）。

    每条边被扩展为一个薄矩形（2 个三角面），宽度由 line_width 控制。
    面片颜色取自 edge_colors，写入 face 元素的 RGB 属性。
    """
    if len(vertices) == 0:
    ...
    # 将每条边扩展为 4 个新顶点 + 2 个三角面
    extra_verts = []
    extra_vcols = []
    faces = []       # 每个元素 (v0, v1, v2)
    face_colors = [] # 每个面的 RGB (uint8)
...
def write_ply_with_faces(
    ...
    """直接写出已有的三角面片（用于 combine 合并已生成的 PLY）。"""
    if len(vertices) == 0:
        raise ValueError("没有可写入的顶点。")
