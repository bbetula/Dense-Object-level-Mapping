...
class Config:
    ...
    BOUNDARY_IGNORE_CLASS_NAMES = ("tree", "person")
    BOUNDARY_IGNORE_KERNEL_SIZE = 3
    BOUNDARY_IGNORE_ITERATIONS = 1
...
def apply_boundary_ignore_band(
    label_map: np.ndarray,
    target_class_ids: Dict[str, int],
) -> np.ndarray:
    """
    对指定类别构建边界忽略带：边界内外一圈像素直接置为 -1，不参与投票。
    """
    if not target_class_ids:
        return label_map

    kernel_size = Config.BOUNDARY_IGNORE_KERNEL_SIZE
    iterations = Config.BOUNDARY_IGNORE_ITERATIONS
    if kernel_size <= 1 or iterations <= 0:
        return label_map

    kernel = np.ones((kernel_size, kernel_size), dtype=np.uint8)
    ignored_map = label_map.copy()
    for _, class_id in target_class_ids.items():
        class_mask = (label_map == class_id).astype(np.uint8)
        if not np.any(class_mask):
            continue
        dilated = cv2.dilate(class_mask, kernel, iterations=iterations)
        eroded = cv2.erode(class_mask, kernel, iterations=iterations)
        boundary_band = (dilated == 1) & (eroded == 0)
        ignored_map[boundary_band] = -1
    return ignored_map
...
def load_label_map_for_pose(
    ...
    boundary_ignore_class_ids: Dict[str, int],
    ...
    label_map = apply_boundary_ignore_band(label_map, boundary_ignore_class_ids)
...
def main(global_pcd_path: str, img_path: str) -> None:
    ...
    boundary_ignore_class_ids = {
        ...
        for name in Config.BOUNDARY_IGNORE_CLASS_NAMES
    ...
    print(
        f"边界忽略带类别: {sorted(boundary_ignore_class_ids.keys())}, "
        f"kernel={Config.BOUNDARY_IGNORE_KERNEL_SIZE}, iterations={Config.BOUNDARY_IGNORE_ITERATIONS}"
    )
    ...
    try:
        colored_pcd = color_point_cloud_with_majority_vote(
            ...
            boundary_ignore_class_ids=boundary_ignore_class_ids,
    ...
    output_file = os.path.join(res_dir, f"{pcd_stem}_color_boundary_ignore.pcd")
