...
for img_name in all_files:
    ...
    with torch.inference_mode():
        with torch.autocast('cuda', dtype=torch.bfloat16):
            ...
            segmentation_map_vit7b = redirect_ade2city(segmentation_map_vit7b)
    ...
    cv2.imwrite(os.path.join(save_dir, f'{img_name}_original.jpg'), image_bgr)
    print(f"保存到 {os.path.join(save_dir, f'{img_name}_original.jpg')}")
    cv2.imwrite(os.path.join(save_dir, f'{img_name}_mask.jpg'), colored_mask_bgr)
    print(f"保存到 {os.path.join(save_dir, f'{img_name}_mask.jpg')}")
    cv2.imwrite(os.path.join(save_dir, f'{img_name}_overlap.jpg'), overlay)
    print(f"保存到 {os.path.join(save_dir, f'{img_name}_overlap.jpg')}")